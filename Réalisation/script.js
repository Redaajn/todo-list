    ...
    render() {
      let tasksArray = this.state.tasksArray.map((task) => {
        return (
          <Task
            key={task.idtaches}
            value={task.tachename}
            done={task.done}
            onClickClose={this.removeTask.bind(this, task.idtaches)}
            onClickTask={this.markDone.bind(this,task.idtaches,task.done)}
          />
        )
      })
